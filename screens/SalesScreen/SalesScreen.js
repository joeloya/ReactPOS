...
const PrintScreenModal = withModal("RECEIPT")(PrintScreen);
...
const SalesScreen = (props) => {
  ...
  const [lastTicketChange, setLastTicketChange] = useState(0);
  ...
  const onPayPress = (change) => {
    setLastTicketChange(change);
    setIsPaying(false);
    setIsPrinting(true);
  };

  const onCancelPayPress = () => {
    setIsPaying(false);
    setIsPrinting(false);
  };
    ...
    <SalesContext.Provider value={salesScreenAPI}>
      {isPaying ? (
        <PayScreenModal
          ...
          onCancelPress={onCancelPayPress}
          total={invoiceTotal}
        />
      ) : (
        <></>
    ...
    <>
      {products.map((item, index) => (
        <ProductCard
          ...
          itemImageSource={item.imageSource}
...
const CategoriesView = (props) => {
  const { categories, activeCategory, setActiveCategory, products, setProducts } = useContext(
    SalesContext
  );
